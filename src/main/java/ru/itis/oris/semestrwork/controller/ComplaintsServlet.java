...
public class ComplaintsServlet extends HttpServlet {
    ...
    private void createTemplate(HttpServletRequest req, HttpServletResponse resp) {
        ...
        List<Complaint> complaints = complaintsService.getByNameWithUser(name != null ? name : "", user, offset, pageSize);
        int totalPages = complaintsService.countResultWithUser(name != null ? name : "", user);
        ...
        try {
            req.getRequestDispatcher("complaints.ftl").forward(req, resp);
